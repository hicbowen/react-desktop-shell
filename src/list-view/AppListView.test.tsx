...
describe('AppListView', () => {
  ...
  it('invokes with Enter and Space and isolates trailing buttons', () => { const invoke = vi.fn(); render({ activationMode: 'invoke', onItemInvoke: invoke }); expect(rows()[0]?.getAttribute('role')).toBe('button'); key(rows()[0]!, 'Enter'); key(rows()[0]!, ' '); expect(invoke).toHaveBeenCalledTimes(2); act(() => host.querySelector('button')?.click()); expect(invoke).toHaveBeenCalledTimes(2) })
  it('only handles Enter in selection mode when invoke is supplied', () => { const invoke = vi.fn(); render({ selectionMode: 'single' }); const event = new KeyboardEvent('keydown', { bubbles: true, cancelable: true, key: 'Enter' }); act(() => rows()[0]!.dispatchEvent(event)); expect(event.defaultPrevented).toBe(false); render({ selectionMode: 'single', onItemInvoke: invoke }); key(rows()[0]!, 'Enter'); expect(invoke).toHaveBeenCalledWith('a') })
