...
export const AppTextArea = forwardRef<HTMLTextAreaElement, AppTextAreaProps>(function AppTextArea({ 'aria-describedby': ariaDescribedBy, 'aria-invalid': ariaInvalid, autoResize = false, className, defaultValue, disabled, fullWidth = false, id, invalid, maxRows, minRows = 2, onChange, readOnly = false, required, resize = 'vertical', showCount = false, value, ...rest }, forwardedRef) {
  ...
  const frameClasses = ['app-text-area__frame', 'app-input-frame', resolvedInvalid ? 'app-input-frame--invalid' : '', resolvedDisabled ? 'app-input-frame--disabled' : '', readOnly ? 'app-input-frame--readonly' : ''].filter(Boolean).join(' ')
  return <span className={classes}><span className={frameClasses}><textarea {...rest} aria-describedby={ariaDescribedBy ?? field?.describedBy} aria-invalid={resolvedInvalid || undefined} className="app-text-area__control" defaultValue={value == null ? defaultValue : undefined} disabled={resolvedDisabled} id={id ?? field?.controlId} onChange={handleChange} readOnly={readOnly} ref={setRef} required={resolvedRequired} rows={minRows} style={{ resize: autoResize ? 'none' : resize, ...rest.style }} value={value} />{showCount ? <span aria-live="polite" className="app-text-area__count">{length}{rest.maxLength != null ? ` / ${rest.maxLength}` : ''}</span> : null}</span></span>
})
