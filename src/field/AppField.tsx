...
export function AppField({ children, className, description, disabled = false, error, htmlFor, id, label, labelAlign = 'start', labelWidth, messageId, orientation = 'vertical', required = false, requiredIndicator = '*', style }: AppFieldProps) {
  const { messages } = useAppLocale()
  const generatedId = useId()
  const controlId = htmlFor ?? id ?? `${generatedId}-control`
  const resolvedMessageId = messageId ?? `${generatedId}-message`
  const classes = ['app-field', `app-field--${orientation}`, `app-field--label-${labelAlign}`, disabled ? 'app-field--disabled' : '', error ? 'app-field--error' : '', className].filter(Boolean).join(' ')
  ...
  return <div aria-disabled={disabled || undefined} className={classes} role="group" style={mergedStyle}>
    <div className="app-field__label-wrap"><label className="app-field__label" htmlFor={controlId}>{label}{required ? <><span aria-hidden="true" className="app-field__required">{requiredIndicator}</span><span className="app-field__sr">{messages.common.required}</span></> : null}</label></div>
